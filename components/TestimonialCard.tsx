import { Testimonial } from '@/types'

interface TestimonialCardProps {
  testimonial: Testimonial
}

export default function TestimonialCard({ testimonial }: TestimonialCardProps) {
  const customerName = testimonial.metadata?.customer_name || ''
  const customerTitle = testimonial.metadata?.customer_title || ''
  const companyName = testimonial.metadata?.company_name || ''
  const companyLogo = testimonial.metadata?.company_logo
  const testimonialQuote = testimonial.metadata?.testimonial_quote || ''
  const rating = testimonial.metadata?.rating?.key || '5'
  const featured = testimonial.metadata?.featured || false

  const renderStars = (rating: string) => {
    const stars = parseInt(rating) || 5
    return (
      <div className="flex items-center mb-3">
        {[...Array(5)].map((_, i) => (
          <svg 
            key={i}
            className={`w-4 h-4 ${i < stars ? 'text-yellow-400' : 'text-gray-200'}`}
            fill="currentColor" 
            viewBox="0 0 20 20"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
    )
  }

  return (
    <div className={`bg-white rounded-xl shadow-lg border p-6 card-hover ${featured ? 'border-primary' : 'border-gray-200'}`}>
      {featured && (
        <div className="flex items-center mb-4">
          <span className="px-2 py-1 bg-primary text-white text-xs font-medium rounded-full">
            Featured
          </span>
        </div>
      )}
      
      {renderStars(rating)}
      
      <blockquote className="text-gray-700 mb-6 leading-relaxed">
        "{testimonialQuote}"
      </blockquote>
      
      <div className="flex items-center">
        {companyLogo && (
          <img 
            src={`${companyLogo.imgix_url}?w=48&h=48&fit=crop&auto=format,compress`}
            alt={companyName}
            className="w-10 h-10 rounded-lg mr-3"
            width={40}
            height={40}
          />
        )}
        <div>
          <div className="font-medium text-gray-900">{customerName}</div>
          <div className="text-sm text-gray-600">
            {customerTitle && `${customerTitle}, `}{companyName}
          </div>
        </div>
      </div>
    </div>
  )
}